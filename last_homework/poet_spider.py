...
import pymysql
from last_homework.poem import Book, Poet
...
def get_data(content, db, cur):
    ...
    for i in range(len(name_list)):
        ...
        # 将爬取到的目标数据封装成封装类进行传递
        poet_obj = Poet(name, dynasty, synopsis)
        put_data_into_database(poet_obj, db, cur)


def get_content_two_data(content_two):
    """
    根据传递过来的第二页的网页源码，获取要爬取的数据
    :param content_two:
    :return: 诗人的简介
    """
...
def put_data_into_database(poet_obj, db, cur):
    """
     将数据插入到数据库中
    :param name:
    :param author:
    ...
    try:
        # 插入一条语句
        sql = "INSERT INTO poet value(null,%s,%s,%s)"
        value = (poet_obj.name, poet_obj.dynasty, poet_obj.synopsis)
        # 执行sql语句
